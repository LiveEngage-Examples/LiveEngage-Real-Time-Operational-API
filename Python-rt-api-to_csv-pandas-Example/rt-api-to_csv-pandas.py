...
accountNumber = parsed_yaml_file['account_number']

# Get these from the LiveEngage API management area
consumer_key = parsed_yaml_file['api_keys']['consumer_key']
consumer_secret = parsed_yaml_file['api_keys']['consumer_secret']
access_token = parsed_yaml_file['api_keys']['access_token']
access_token_secret = parsed_yaml_file['api_keys']['access_token_secret']
...
if skillIDs:
	...
	for skillData in skillResults:
		if str(skillData['id']) in skillIDs:
			df_.at[str(skillData['id']), "skill_name"] = skillData['name']

	# Insert our date and time info into the dataframe using our start_time_epoch
	for skill in skillIDs:
		df_.at[skill, "date"] = myDate
		df_.at[skill, "timeframe"] = myTimeFrame
		df_.at[skill, "timezone"] = myTimeZone
	...
	for skill in queueHealthResults['skillsMetrics']:
		...
		### dataframe.at[index, column] = myDataToPutThere
		if chats:
			df_.at[skill, "chats"] = chats
		else:
			df_.at[skill, "chats"] = 0
		if chatsAnswered:
			df_.at[skill, "chats_answered"] = chatsAnswered
		else:
			df_.at[skill, "chats_answered"] = 0
		df_.at[skill, "abandoned"] = abandoned
		if avgTimeToAnswer:
			df_.at[skill, "average_speed_to_answer"] = avgTimeToAnswer
		else:
			df_.at[skill, "average_speed_to_answer"] = 0
	...
	for skill in engActivityResults['skillsMetricsPerAgent']['metricsPerSkill']:
		...
		if totalHandlingTime:
			df_.at[skill, "total_handling_time"] = totalHandlingTime
		else:
			df_.at[skill, "total_handling_time"] = 0
		...
		df_.at[skill, "average_handling_time"] = avg_handling_time
	...
	for skill in engActivityResults['skillsMetricsPerAgent']['metricsPerSkill']:
		...
		df_.at[skill, "staff"] = total_active_agents
	...
	df_.at["N/A", "skill_name"] = "All Offline"
	df_.at["N/A", "date"] = myDate
	df_.at["N/A", "timeframe"] = myTimeFrame
	df_.at["N/A", "timezone"] = myTimeZone
	df_.at["N/A", "staff"] = 0
	df_.at["N/A", "chats"] = 0
	df_.at["N/A", "chats_answered"] = 0
	df_.at["N/A", "abandoned"] = 0
	df_.at["N/A", "average_speed_to_answer"] = 0
	df_.at["N/A", "total_handling_time"] = 0
	df_.at["N/A", "average_handling_time"] = 0
